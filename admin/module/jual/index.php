				...
				<div class="card-body">
					<div id="keranjang" class="table-responsive">
						<table class="table table-bordered">
							<tr>
								<td><b>Tanggal</b></td>
								<td><input type="text" readonly="readonly" class="form-control" value="<?php echo date("j F Y, G:i");?>" name="tgl"></td></tr>
							</tr>
						</table>
        ...
        if (!empty($_GET['nota']) && $_GET['nota'] === 'yes') {
            ...
            if (!empty($bayar)) {
                ...
                if ($bayar >= $total) {
                    ...
                    $pelanggan = isset($_POST['pelanggan']) ? $_POST['pelanggan'] : array();
                    $alamat = isset($_POST['alamat']) ? $_POST['alamat'] : array();
                    $hp = isset($_POST['hp']) ? $_POST['hp'] : array();
                    ...
                    for ($x = 0; $x < $jumlah_dipilih; $x++) {
                        $d = array(
                            ...
                            $tgl_input[$x],
                            $periode[$x],
                            isset($pelanggan[$x]) ? $pelanggan[$x] : '', // Menghapus htmlspecialchars()
                            isset($alamat[$x]) ? $alamat[$x] : '', // Menghapus htmlspecialchars()
                            isset($hp[$x]) ? $hp[$x] : '' // Menghapus htmlspecialchars()
                        );
                        
                        // Query untuk memasukkan data ke dalam tabel nota
...
<form method="POST" action="index.php?page=jual&nota=yes#kasirnya">
    ...
    <tr>
        <td><input type="text" class="form-control" name="pelanggan[]" id="pelanggan" placeholder="Masukkan Nama Pelanggan" required></td>
        <td><input type="text" class="form-control" name="hp[]" id="hp" placeholder="Masukkan No Hp Pelanggan" required></td>
        <td><textarea class="form-control" name="alamat[]" id="alamat" placeholder="Masukkan Alamat Pelanggan" required></textarea></td>
    </tr>
    ...
    <tr>
        <td>Total Semua  </td>
        <td><input type="text" readonly="readonly" class="form-control" name="total" value="<?php echo $total_bayar;?>"></td>
        <td>Uang Tunai  </td>
