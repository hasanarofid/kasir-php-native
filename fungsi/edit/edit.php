...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['pengaturan'])) {
        ...
        $data[] = $email;
        $data[] = $id;
        $sql = 'UPDATE toko SET nama_toko=?, alamat_toko=?, tlp=?, nama_pemilik=?, email=? WHERE id_toko = ?';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['service'])) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        echo '<script>window.location="../../index.php?page=data&success=edit-data"</script>';
